Shader "NatureManufacture/Particles/Smoke Normalmap Unlit"
{
	...
	SubShader
	{
		...
		void surf( Input i , inout SurfaceOutput o )
		{
			...
			float localGetLightData40_g11 = ( 0.0 );
			float3 ase_worldPos = i.worldPos;
			float3 Position40_g11 = ase_worldPos;
			float3 lightDir40_g11 = float3( 0,0,0 );
			float3 color40_g11 = float3( 0,0,0 );
			{
			color40_g11 = float3(0, 0, 0);
			float4 direction = _WorldSpaceLightPos0;
			if(direction.w == 0)
			 lightDir40_g11 = _WorldSpaceLightPos0;
			else
			 lightDir40_g11 = Position40_g11 -_WorldSpaceLightPos0;
			        color40_g11 = _LightColor0.rgb;
			lightDir40_g11 =normalize(lightDir40_g11);
			  
			}
			float3 clampResult45_g11 = clamp( color40_g11 , float3( 0.01,0.01,0.01 ) , float3( 1000000,1000000,1000000 ) );
			float3 normalizeResult166 = normalize( clampResult45_g11 );
			float3 lerpResult167 = lerp( appendResult169 , normalizeResult166 , _Light_Blend_Intensity);
			float3 clampResult44_g11 = clamp( lightDir40_g11 , float3( -1,-1,-1 ) , float3( 1,1,1 ) );
			#ifdef _Use_Scene_Light_s_Direction
				float3 staticSwitch178 = clampResult44_g11;
			#else
			...
			float3 break31_g10 = UnpackNormal( SAMPLE_TEXTURE2D( _Normal_Map, sampler_Normal_Map, i.uv_texcoord.xy ) );
			...
			#endif
			o.Emission = ( ( float4( ( ( lerpResult167 * appendResult197 ) + appendResult176 ) , 0.0 ) * ( staticSwitch293 * clampResult201 ) ) + staticSwitch208 ).rgb;
