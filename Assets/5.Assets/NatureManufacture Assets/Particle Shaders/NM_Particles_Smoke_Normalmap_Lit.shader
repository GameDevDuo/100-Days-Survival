Shader "NatureManufacture/Particles/Smoke Normalmap Lit"
{
	Properties
	{
		[Toggle(_Use_Scene_Light_s_Direction)] _Use_Scene_Light_s_Direction("Use Scene Light's Direction", Float) = 1
		...
		_Color_Mask_R_Emission_B_Transparency_A("Color Mask (R) Emission (B) Transparency (A)", 2D) = "white" {}
		[Toggle(_Read_Color_Mask)] _Read_Color_Mask("Read Color Mask", Float) = 1
		_Particle_Color_RGB_Alpha_A("Particle Color (RGB) Alpha (A)", Color) = (1,1,1,1)
		_Color_Blend("Color Blend", Range( 0 , 1)) = 1
		...
		[Toggle(_Emission_From_R_T_From_B_F)] _Emission_From_R_T_From_B_F("Emission From R (T) From B (F)", Float) = 0
	...
	SubShader
	{
		...
		#pragma shader_feature_local _Read_Color_Mask
		...
		UNITY_DECLARE_TEX2D_NOSAMPLER(_Normal_Map);
		SamplerState sampler_Normal_Map;
		uniform float _Normal_Map_Strength;
		...
		uniform float _Normal_Map_Light_Impact;
		uniform float _Light_Intensity;
		uniform float4 _Shadow_Color;
		uniform float4 _Particle_Color_RGB_Alpha_A;
		UNITY_DECLARE_TEX2D_NOSAMPLER(_Color_Mask_R_Emission_B_Transparency_A);
		SamplerState sampler_Color_Mask_R_Emission_B_Transparency_A;
		...
		{
			float3 tex2DNode287 = UnpackNormal( SAMPLE_TEXTURE2D( _Normal_Map, sampler_Normal_Map, i.uv_texcoord.xy ) );
			float3 break31_g6 = tex2DNode287;
			float2 appendResult35_g6 = (float2(break31_g6.x , break31_g6.y));
			float temp_output_38_0_g6 = _Normal_Map_Strength;
			float lerpResult36_g6 = lerp( 1.0 , break31_g6.z , saturate( temp_output_38_0_g6 ));
			float3 appendResult34_g6 = (float3(( appendResult35_g6 * temp_output_38_0_g6 ) , lerpResult36_g6));
			o.Normal = appendResult34_g6;
			float3 appendResult169 = (float3(_Light_Color.rgb));
			float localGetLightData40_g9 = ( 0.0 );
			float3 ase_worldPos = i.worldPos;
			float3 Position40_g9 = ase_worldPos;
			float3 lightDir40_g9 = float3( 0,0,0 );
			float3 color40_g9 = float3( 0,0,0 );
			{
			color40_g9 = float3(0, 0, 0);
			float4 direction = _WorldSpaceLightPos0;
			if(direction.w == 0)
			 lightDir40_g9 = _WorldSpaceLightPos0;
			else
			 lightDir40_g9 = Position40_g9 -_WorldSpaceLightPos0;
			        color40_g9 = _LightColor0.rgb;
			lightDir40_g9 =normalize(lightDir40_g9);
			  
			}
			float3 clampResult45_g9 = clamp( color40_g9 , float3( 0.01,0.01,0.01 ) , float3( 1000000,1000000,1000000 ) );
			float3 normalizeResult166 = normalize( clampResult45_g9 );
			float3 lerpResult167 = lerp( appendResult169 , normalizeResult166 , _Light_Blend_Intensity);
			float3 clampResult44_g9 = clamp( lightDir40_g9 , float3( -1,-1,-1 ) , float3( 1,1,1 ) );
			#ifdef _Use_Scene_Light_s_Direction
				float3 staticSwitch178 = clampResult44_g9;
			#else
				float3 staticSwitch178 = _Light_Direction;
			#endif
			float3 ase_worldTangent = WorldNormalVector( i, float3( 1, 0, 0 ) );
			float3 worldToObjDir277 = mul( unity_WorldToObject, float4( ase_worldTangent, 0 ) ).xyz;
			float3 ase_worldBitangent = WorldNormalVector( i, float3( 0, 1, 0 ) );
			float3 worldToObjDir278 = mul( unity_WorldToObject, float4( ase_worldBitangent, 0 ) ).xyz;
			float3 ase_worldNormal = WorldNormalVector( i, float3( 0, 0, 1 ) );
			float3 worldToObjDir279 = mul( unity_WorldToObject, float4( ase_worldNormal, 0 ) ).xyz;
			float3 break31_g10 = tex2DNode287;
			float2 appendResult35_g10 = (float2(break31_g10.x , break31_g10.y));
			float temp_output_38_0_g10 = _Normal_Map_Light_Impact;
			float lerpResult36_g10 = lerp( 1.0 , break31_g10.z , saturate( temp_output_38_0_g10 ));
			float3 appendResult34_g10 = (float3(( appendResult35_g10 * temp_output_38_0_g10 ) , lerpResult36_g10));
			float dotResult272 = dot( staticSwitch178 , mul( float3x3(worldToObjDir277, worldToObjDir278, worldToObjDir279), appendResult34_g10 ) );
			float temp_output_290_0 = ( ( dotResult272 + 1.0 ) * 0.5 );
			float4 temp_cast_1 = (( temp_output_290_0 * _Light_Intensity )).xxxx;
			...
			float4 tex2DNode296 = SAMPLE_TEXTURE2D( _Color_Mask_R_Emission_B_Transparency_A, sampler_Color_Mask_R_Emission_B_Transparency_A, i.uv_texcoord.xy );
			float4 temp_cast_5 = (tex2DNode296.r).xxxx;
			float4 lerpResult297 = lerp( temp_cast_5 , _Particle_Color_RGB_Alpha_A , _Color_Blend);
			#ifdef _Read_Color_Mask
				float4 staticSwitch293 = lerpResult297;
			#else
				float4 staticSwitch293 = _Particle_Color_RGB_Alpha_A;
			#endif
			float4 clampResult201 = clamp( i.vertexColor , float4( 0,0,0,0 ) , float4( 1,1,1,1 ) );
			o.Albedo = ( float4( ( ( lerpResult167 * appendResult197 ) + appendResult176 ) , 0.0 ) * ( staticSwitch293 * clampResult201 ) ).rgb;
			float temp_output_216_0 = saturate( ( pow( ( ( i.uv_texcoord.z * _Emission_Gradient_Contrast ) - _Emission_Over_Time ) , 3.0 ) * -1.0 ) );
				...
				float staticSwitch219 = ( 1.0 - tex2DNode296.r );
			#else
				float staticSwitch219 = tex2DNode296.b;
			#endif
			float2 appendResult203 = (float2(temp_output_290_0 , 0.0));
			...
			#endif
			o.Emission = staticSwitch208.rgb;
			o.Metallic = _Metallic;
			o.Smoothness = _Smoothness;
			o.Occlusion = _Ambient_Occlusion;
			float temp_output_237_0 = ( ( ( tex2DNode296.a * clampResult201.a ) * _Alpha_Multiplier ) * i.vertexToFrag254 );
